...
export default function PropertyReviews({ reviews }: PropertyReviewsProps) {
  const { averageRating, totalReviews, reviews: reviewList } = reviews;
  ...
  return (
    <div className="rounded-2xl border bg-card p-4 md:p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-medium text-primary">Tenant feedback</p>

          <h2 className="mt-1 text-2xl font-bold">Reviews</h2>
        </div>
      ...
      {totalReviews === 0 ? (
        <div className="mt-6 flex flex-col items-center justify-center rounded-xl border border-dashed py-10 text-center">
          ...
          <p className="mt-1 max-w-sm text-sm text-muted-foreground">
            Reviews from tenants will appear here once someone has rented this
            property.
          </p>
        </div>
      ) : (
        <div className="mt-6 space-y-4">
          {reviewList.map((review) => (
            <div key={review.id} className="rounded-xl border p-4 sm:p-5">
